<template>
  <div>
    <div style="margin-top: 15px">
      <el-input placeholder="请输入内容" v-model="searchKey" class="input-with-select" style="float:left;width:35%"
        ...
        <el-select v-model="searchValue" slot="prepend" placeholder="请选择" style="width:7.5em">
          <el-option label="截止时间" value="1"></el-option>
    ...
    <el-table :data="taskData.slice((currentPage-1)*pageSize,currentPage*pageSize)"
      :default-sort="{prop: '', order: 'descending'}" style="width: 100%; overflow:hidden;">
      ...
      <el-table-column prop="deadLine" label="截止时间" sortable width="150em">
      ...
      <el-table-column label="任务标题" width="300em">
        <template slot-scope="scope">
      ...
      <el-table-column label="发布者" width="150em" prop="name1">
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
      ...
      <el-table-column label="任务状态" width="150em">
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
      ...
      <el-table-column label="上传文件" width="150em">
        <el-upload class="name-wrapper" ref="upload" :with-credentials="true" :limit="1" :auto-upload="true"
          accept=".doc,.docx,.txt" :action="this.$http.defaults.baseURL+'/upload'" :on-success="afterUpload">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
            <el-button @click.native.prevent="submitTask(scope.row._id, scope.row)" id="btn" size="medium"
              type="primary" plain>提交任务</el-button>
    ...
    <div style="text-align: center;margin-top: 30px;">
      <el-pagination background layout="prev, pager, next" :total="total" :page-size="pageSize"
        :current-page.sync="currentPage" @current-change="current_change">
      </el-pagination>
    </div>
      ...
      searchKey () {
        if (this.searchKey === '') {
          ...
          if (this.searchValue === '1') {
            ...
            for (let i = 0; i < this.searchData.length; i++) {
              if (this.searchData[i].taskStatus.match(this.searchKey)) { this.taskData.push(this.searchData[i]) }
            }
          } else {
            alert('请先选择要搜索的内容')
          }
      ...
      fetch () {
        ...
        this.$http.get("/task").then(res => {
          ...
          for (var i = 0; i < res.data.length; i++) {
            if (res.data[i].getter == localStorage.name) {

<template>
  <div>
    ...
    <el-table :data="taskData.slice((pageNo-1)*pageSize,pageNo*pageSize)" style="width: 100%"
      ...
      <el-table-column label="任务标题" width="450em">
        <template slot-scope="scope">
      ...
      <el-table-column label="发布者" width="200em">
        <template slot-scope="scope">
          <div class="publisher">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button @click.native.prevent="getTask(scope.row._id, scope.row)" size="medium" type="primary" plain>领取任务
          </el-button>
          <!-- <el-button
            size="medium"
            type="primary"
    ...
    <div class="block">
      <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange"
        :current-page.sync="pageNo" :page-size="pageSize" layout="prev, pager, next, jumper" :total="total">
      </el-pagination>
    </div>
  </div>
</template>
...
export default {
  data () {
    return {
      total: 1,
      pageNo: Number(localStorage.getItem('pagination')) || 1,
      pageSize: 7,
      searchKey: '',
      searchValue: '',
      taskData: [],
  ...
  methods: {
    ...
    // },
    getTask (id, row) {
      if (row.publisher == localStorage.name) {
        this.$message({
          message: '请勿领取自己发布的任务',
          type: 'warning'
        })
        ...
        row.getterEmail = localStorage.email;
        console.log(id);
        console.log(row);
        console.log(this.taskData);
